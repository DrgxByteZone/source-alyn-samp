...
public abstract class AbstractC2123qd {
    ...
    public static int a(double d, double d2, double d3) {
        ...
        if (round < 0) {
            ...
            min = Math.min(round, JfifUtil.MARKER_FIRST_BYTE);
        }
        int round2 = (int) Math.round(d5 * 255.0d);
        if (round2 < 0) {
            min2 = 0;
        } else {
            min2 = Math.min(round2, JfifUtil.MARKER_FIRST_BYTE);
        }
        int round3 = (int) Math.round(d6 * 255.0d);
        if (round3 >= 0) {
            i = Math.min(round3, JfifUtil.MARKER_FIRST_BYTE);
        }
    ...
    public static int c(int i, int i2) {
        ...
        int i3 = 255 - (((255 - alpha2) * (255 - alpha)) / JfifUtil.MARKER_FIRST_BYTE);
    ...
    public static int d(int i, int i2, int i3, int i4, int i5) {
        ...
        return (((255 - i2) * (i3 * i4)) + ((i * JfifUtil.MARKER_FIRST_BYTE) * i2)) / (i5 * JfifUtil.MARKER_FIRST_BYTE);
    }
