...
public final class Tasks {
    ...
    public static <TResult> TResult await(Task<TResult> task) throws ExecutionException, InterruptedException {
        AbstractC0378Ls.g("Must not be called on the main application thread");
        Looper myLooper = Looper.myLooper();
        if (myLooper != null && Objects.equals(myLooper.getThread().getName(), "GoogleApiHandler")) {
            throw new IllegalStateException("Must not be called on GoogleApiHandler thread.");
        }
        AbstractC0378Ls.i(task, "Task must not be null");
        if (task.isComplete()) {
    ...
    public static <T> Task<T> withTimeout(Task<T> task, long j, TimeUnit timeUnit) {
        ...
        AbstractC0378Ls.b("Timeout must be positive", z);
        AbstractC0378Ls.i(timeUnit, "TimeUnit must not be null");
        final zzb zzbVar = new zzb();
        final TaskCompletionSource taskCompletionSource = new TaskCompletionSource(zzbVar);
        final HandlerC1523j90 handlerC1523j90 = new HandlerC1523j90(Looper.getMainLooper());
        handlerC1523j90.postDelayed(new Runnable() { // from class: com.google.android.gms.tasks.zzx
        ...
        task.addOnCompleteListener(new OnCompleteListener() { // from class: com.google.android.gms.tasks.zzy
            ...
            public final void onComplete(Task task2) {
                HandlerC1523j90.this.removeCallbacksAndMessages(null);
    ...
    public static <TResult> Task<TResult> call(Executor executor, Callable<TResult> callable) {
        AbstractC0378Ls.i(executor, "Executor must not be null");
        AbstractC0378Ls.i(callable, "Callback must not be null");
        zzw zzwVar = new zzw();
